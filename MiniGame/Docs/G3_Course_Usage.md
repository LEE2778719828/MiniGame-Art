...
AC/BC 由 Day 的 `DT_GameStages.ForkPair` 传入，主段由
`DT_GameStages.DefaultRoute` 选择 `DA_Rules.RouteModes[A/B/C]`；C 路线由同一套 `BranchRoutes[C]` 和
...
AB/AC/BC 的岔口视觉由 `DA_Course.ForkAtomMap` 指定的
`ANightCourseForkAtomActor` 子类直接生成；其左右出口分别连接所选 A/B/C 分支，
路牌和桥梁应作为组合 Blueprint 的内部组件配置。
...
| C 掉料 | `EnterDropCount=2`；M01-M05 ID 由 `DA_Course.FoeDropMap` 固定绑定，`DropCycle` 仅兼容未映射敌人 |
| C 带出 | `CarryOutBonus=0.3` |
| 换键 | 示例为 L2 两次、L3 三次；必须在 `DA_Course.KeySwapCues` 或关卡覆盖行中手动配置 |
...
| 分类 | 字段 |
|---|---|
| 基础段 | `DT_GameStages.DefaultRoute` + `DA_Rules.RouteModes[A/B/C]` / `DA_Atoms` / `DA_Course.AdvanceSpeed` |
| 岔口 | `bEnableFork` / `ForkTimeoutSeconds` / `bForkTimeoutPickLeft` / `BranchEnterBufferSeconds` / `BranchEntryGapCm` / `ForkAtomMap[AB/AC/BC]` |
...
| 关卡选择 | `DT_GameStages.DefaultRoute` 与 `ForkPair` → Night Bootstrap；`PreviewRoute` 仅用于编辑器预览 |
...
1. 在 `DT_GameStages` 将 L2 设置为 `DefaultRoute=A`、`ForkPair=AC`，并确认 `DA_Course` 已绑定 `DA_Rules`、`DA_Atoms` 和 A/C Branch Atom queues
2. PIE → 打完基础段 → **E 选 C**
...
- `DT_GameStages.DefaultRoute/ForkPair` / `CourseRuleData.RouteModes` / `BranchRoutes[C]`
