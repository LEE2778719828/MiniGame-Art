...
| `routeModes` | Day 的 `DefaultRoute` 选择的主段模板池；每个 A/B/C 队列有自己的 `targetAtomCount` 和 `atoms` |
| `branchRoutes` | A/B/C 分支模板池对象；每个对象用 `targetAtomCount` 控制生成总数 |
| `forkAfterBaseAtomIndex` | 所选主段队列中进入岔口前保留的 Atom 数量 |
...
旧 JSON 中的 `baseRoute/baseAtomCount` 仅在导入时迁移到 `routeModes.A`，新导出不会包含它们。
